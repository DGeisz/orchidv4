...
impl WsCommandAdapterPort for WsCommandAdapter {
    fn consume_ws_command(&mut self, ws_command: WsCommand) -> (WsResponse, bool) {
        match ws_command {
            ...
            WsCommand::GetOpenFiles => match self.curator.get_open_files() {
                Some(open_files) => (WsResponse::OpenFiles(open_files), false),
                None => NO_OP_RES,
            },
            ...
            WsCommand::SaveOpenFolders { open_folders } => {
                match self.curator.save_open_folders(open_folders) {
                    Ok(oft) => (WsResponse::OFT(oft), false),
                    Err(_) => NO_OP_RES,
                }
            }
            WsCommand::SaveOpenFiles { open_files } => {
                match self.curator.save_open_files(open_files) {
                    Ok(oft) => (WsResponse::OFT(oft), false),
                    Err(_) => NO_OP_RES,
                }
            }
